...
const ActivityCalendar: React.FC = () => {
  ...
  const months = Array.from(
    new Set(daysArray.map(d => format(d, 'MMM')))
  );
    ...
    <div className="p-6 bg-white rounded-lg shadow-sm border">
      ...
      <div className="calendar-heatmap">
        <div className="flex mb-2">
          {months.map((month, i) => (
            <div key={month} className="flex-1 text-xs text-center text-muted-foreground">
              {month}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: parseISO(activityData[0].date).getDay() }).map((_, i) => (
          ...
          {activityData.map((day) => (
            <div 
              key={day.date} 
              className={`day h-3 w-3 rounded-sm cursor-pointer transition-all ${getColorIntensity(day.count)}`}
              title={`${day.date}: ${day.count} recording${day.count !== 1 ? 's' : ''}`}
            />
          ))}
        </div>
      </div>
